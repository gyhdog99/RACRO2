            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        # # bsz = len(batch)
                        # gen_lookahead_batch = deepcopy(gen_batch)
                        # lookahead_g = self.config['actor_rollout_ref']['rollout']['lookahead_g']
                        # gen_lookahead_batch.meta_info['sampling_n'] = lookahead_g
                        # gen_lookahead_output = self.actor_rollout_wg.generate_sequences(gen_lookahead_batch)
                    
                        # batch_lookahead = batch.repeat(repeat_times=lookahead_g, interleave=True)
                        # batch_lookahead = batch_lookahead.union(gen_lookahead_output)
                        
                        # correctness = self.reward_fn(batch_lookahead, return_dict=True)['reward_extra_info']['correctness']
                        # keep = (np.array(correctness).reshape(-1, lookahead_g).sum(axis=1) < lookahead_g)

                        
                        # keep_idx = keep.nonzero()[0]
                        # num_true_items = len(keep_idx)
                        # print("keep1:", num_true_items)
                        # adjusted_num_true_items = (num_true_items // 8) * 8
                        # print("keep2:", adjusted_num_true_items)
                        # keep_idx_adjusted = keep_idx[:adjusted_num_true_items]
                        # # batch.pop(batch_keys=list(gen_baseline_output.batch.keys()))

                        # batch = batch.select_idxs(keep_idx_adjusted)
                        # gen_batch = gen_batch.select_idxs(keep_idx_adjusted)

                        # del gen_lookahead_batch, gen_lookahead_output, batch_lookahead
                        

                        # # rollout_G = self.config['actor_rollout_ref']['rollout']['n']
                        # # gen_batch.meta_info['sampling_n'] = int(bsz * (rollout_G - lookahead_g) // keep.sum())
